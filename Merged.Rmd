...
---
##Introduction
In the scope of our project we mainly focused on the research question: What factors lead to customer satisfaction for an Airline?
...
```
#Christian 
#After analyzing the first inspections on our dataset we initiated our further evaluation with a generalized linear model. It fits in linear, logistic regression models and in cases where the response variable has an error distribution that is non-normal. 
...
#Now we are feeding the prediction function with our extracted data.
```{r}
